...
export const createStudent = async (
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (school?.activeAcademicYearId && data.classId) {
...
export const createExamTemplate = async (
  ...
  try {
    ...
    const term = await prisma.term.findFirst({
      where: { id: data.termId, schoolId: schoolId },
      select: { id: true },
    });
    if (!term) {
      return { success: false, error: true, message: "Term not found for this school." };
    }
    ...
    if (!classExists) {
      return { success: false, error: true, message: "Selected class not found in this school." };
    }

    const subjectExists = await prisma.subject.findFirst({
      where: { id: data.subjectId, schoolId: schoolId },
      select: { id: true },
    });
    if (!subjectExists) {
      return { success: false, error: true, message: "Selected subject not found in this school." };
    }
    ...
    if (data.teacherId) {
      ...
      if (!teacherExists) {
        return { success: false, error: true, message: "Selected teacher not found in this school." };
      }
    }
    ...
    if (data.roomId) {
      const roomExists = await prisma.room.findFirst({
        where: { id: data.roomId, schoolId: schoolId },
        select: { id: true },
      });
      if (!roomExists) {
        return { success: false, error: true, message: "Selected room not found in this school." };
      }
    }
    ...
    revalidatePath(`/schools/${schoolId}/list/exams`);
...
export const updateExamTemplate = async (
  ...
  if (!data.id) {
    return { success: false, error: true, message: "Exam template ID missing." };
  }
  try {
    const schoolId = await getCurrentUserSchoolId();
    ...
    if (!examTemplateExists) {
      return { success: false, error: true, message: "Exam template not found in this school." };
    }
...
export const deleteExamTemplate = async (
  ...
  if (!id) return { success: false, error: true, message: "Exam template ID missing." };
  try {
    const schoolId = await getCurrentUserSchoolId();

    const exists = await prisma.examTemplate.findFirst({
      where: { id: id, schoolId: schoolId },
      select: { id: true },
    });
    if (!exists) return { success: false, error: true, message: "Exam template not found." };
    ...
    revalidatePath(`/schools/${schoolId}/admin/schedule`);
    return { success: true, error: false, message: "Exam template deleted." };
  } catch (err) {
    console.error("Error deleting exam template:", err);
    return { success: false, error: true, message: err instanceof Error ? err.message : "Failed to delete exam template." };
  }
};
    ...
    if (userId && userEmail) { // Scenario 1: Existing authenticated user
      ...
      await prisma.$transaction(async (tx) => {
        ...
        const newAuthUser = await tx.auth.create({
          data: {
            username: authUsernameForNewUser,
            email: email,
            password: hashedPassword,
            role: 'admin',
            schoolId: newSchoolId,
        ...
        const tokenPayload: AuthUser = {
          ...
          role: 'admin',
          schoolId: newSchoolId,
          accountType: newAuthUser.accountType,
