...
import { timetableAssistantBodySchema } from "@/lib/formValidationSchemas";
import { requireSchoolAccess, type AuthUser } from "@/lib/auth";
import { runTimetableAssistantPreviewOptimize } from "@/lib/timetableAssistantService";
...
export async function POST(
  ...
  const parsed = timetableAssistantBodySchema.safeParse(body);
  if (!parsed.success) {
  ...
  const result = await runTimetableAssistantPreviewOptimize({ schoolId, body: parsed.data });
  if (!result.ok) {
    return NextResponse.json(
